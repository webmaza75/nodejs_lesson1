...
var action = {
    ...
    changeNews: function (id, newTitle, newDescription, callback) {
        pool.getConnection(function (err, connection) {
            if (err)
                throw err;

            connection.query('update news set title = ? description = ? where id = ?;', [newTitle, newDescription, id], callback);
            connection.release();
        });
    },
